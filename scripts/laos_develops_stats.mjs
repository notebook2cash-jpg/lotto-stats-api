...
const TARGET_URL = "https://exphuay.com/calculate/laosdevelops";
...
async function scrapeData(url) {
  ...
  const data = await page.evaluate(() => {
    ...
    // ดึงสถิติ 30 งวด - หวยลาวพัฒนา
    const stats30Bottom2 = [];
    const stats30Top3 = [];

    const bottom2Match = bodyText.match(
      /ตารางแสดงผลหวยลาวพัฒนา 2 ตัวล่าง[\s\S]*?เลขที่ออก\s+จำนวนครั้งที่ออก([\s\S]*?)(?:ตาราง|คำนวณ|$)/
    );
    if (bottom2Match) {
    ...
    const top3Match = bodyText.match(
      /ตารางแสดงผลหวยลาวพัฒนา 3 ตัวบน[\s\S]*?เลขที่ออก\s+จำนวนครั้งที่ออก([\s\S]*?)(?:ตาราง|$)/
    );
    if (top3Match) {
...
async function main() {
  ...
  console.log("✅ public/laos_develops.json updated");
